...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { data: ticket, error } = await admin
      ...
      .single()
    if (error || !ticket) throw new Error(error?.message ?? "Não foi possível reabrir o chamado")
    ...
    const { data: message, error: messageError } = await admin
      ...
      .insert({
        ...
        kind: "system",
        author_id: user.id,
        text: `${user.name} reabriu o chamado. Motivo: ${reason}`,
      })
      .select(MESSAGE_SELECT)
      .single()
    if (messageError || !message) throw new Error(messageError?.message ?? "Não foi possível registrar a reabertura")
