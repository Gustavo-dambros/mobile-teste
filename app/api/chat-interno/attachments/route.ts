...
import type { AttachmentKind } from "@/lib/chat-interno/types"
import { requireUser } from "@/lib/chat-interno/server"
import { createAdminClient } from "@/lib/supabase/admin"

// This URL is generated once here and stored permanently in the chat
// message's attachments — never regenerated on read. A 30-day TTL meant
// every attachment silently turned into a broken link a month after upload,
// even though the file was still in Storage. Long-lived instead of
// switching to on-read regeneration (a bigger change across every
// attachment consumer).
...
export async function POST(request: Request) {
  try {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      const durationSeconds = durations[i] ? Number(durations[i]) : undefined
      attachments.push({
