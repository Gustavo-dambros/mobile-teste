...
/** Same pattern as components/atividades-setor/UserMultiSelect.tsx, but for the real (string-id) chat roster. */
export function RosterMultiSelect({
  value,
  onChange,
  options,
  placeholder = "Selecionar pessoas",
  searchPlaceholder = "Nome ou setor...",
    ...
    <div className="flex flex-col gap-2">
      <Popover open={open} onOpenChange={setOpen}>
        ...
        >
          ...
          <ChevronRightIcon className={cn("opacity-50 transition-transform duration-200", open && "rotate-90")} />
        </PopoverTrigger>
            ...
            <CommandList>
              ...
              <CommandGroup>
                {options.map((member) => (
                  <CommandItem
                    key={member.id}
                    value={`${member.name} ${member.sector}`}
